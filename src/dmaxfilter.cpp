...
typedef struct{
  ...
  int threadNumber;
  int numThreads;
} MAX_HUANG_8_THREAD_PARAMS_T;
/// \endcond

#define MAX_MAXFILT_THREADS 64

void* DMaxFilter::DMaxFilter_Huang8threadWrap(void* params){
  MAX_HUANG_8_THREAD_PARAMS_T *pParams;

  pParams = (MAX_HUANG_8_THREAD_PARAMS_T*)params;
  DMaxFilter::maxFiltHuang_u8(*(pParams->pImgDst),
				 *(pParams->pImgSrc),
...
///Default constructor
DMaxFilter::DMaxFilter(){
  DInstanceCounter::addInstance("DMaxFilter");
  _maxFiltType = DMaxFilt_circle;
  rgKern = NULL;
  rgRightEdge = NULL;
  _radiusX = 1;
  _radiusY = 1;
...
DMaxFilter::DMaxFilter(int radiusX, int radiusY, DMaxFiltType filtType){
  DInstanceCounter::addInstance("DMaxFilter");
  _maxFiltType = filtType;
  rgKern = NULL;
  rgRightEdge = NULL;
  _radiusX = radiusX;
  _radiusY = radiusY;
...
///Destructor
DMaxFilter::~DMaxFilter(){
  DInstanceCounter::removeInstance("DMaxFilter");
  if(rgKern){
    free(rgKern);
    rgKern = NULL;
  }
...
///Set the kernel (window) radii for the max filter
/**The actual width (and height) of the window will be 2*radius+1*/
void DMaxFilter::setRadius(int radiusX, int radiusY){
  if((radiusX < 0)||(radiusY < 0)){
    fprintf(stderr, "DMaxFilter::setRadius() negative value not allowed\n");
    return;
  }
  _radiusX = radiusX;
  _radiusY = radiusY;
...
int DMaxFilter::getRadiusX(){
  return _radiusX;
}
///Get the vertical radius of the kernel
int DMaxFilter::getRadiusY(){
  return _radiusY;
}

///Set the number of threads that will be used to do the max filter
/** The max filter may run faster if more than one thread is used,
 ...
 *  seem to speed up the max filter very much.  For example, on a
 ...
 */
void DMaxFilter::setNumThreads(int numThreads){
#ifndef D_NOTHREADS
  if((_numThreads > 0) && (_numThreads <= MAX_MAXFILT_THREADS)){
    _numThreads = numThreads;
  }
  else{
    fprintf(stderr, "DMaxFilter::setNumThreads() numThreads must be from "
	    "1 to %d\n", MAX_MAXFILT_THREADS);
  }
#else
  fprintf(stderr, "DMaxFilter::setNumThreads() thread support not "
	  "compiled");
#endif
}

///return the number of threads that the filter has been set to use
int DMaxFilter::getNumThreads(){
  return _numThreads;
}

///set the type of max filter algorithm
/**By default, RGB and _u8 images will use Huang_circle and all others
 * (including RGB_16 and _u16) will use the slow method */
void DMaxFilter::setType(DMaxFiltType filtType){
  if(rgKern){
    free(rgKern);
    rgKern = NULL;
    free(rgRightEdge);
...
///return the type of the max filter
DMaxFilter::DMaxFiltType DMaxFilter::getType(){
  return _maxFiltType;
}

///private function that actually performs huang max filt on 8-bit images
 ...
 */
void DMaxFilter::maxFiltHuang_u8(DImage &imgDst, const DImage &imgSrc,
  ...
  for(int y = threadNumber; y < h; y += numThreads){
	...
	idx3 += wTmp;
      ...
      // find max from the histogram for DEBUG purposes (remove this later)
      for(lastMax = 255; (lastMax > 0) && (0==rgHist[lastMax]); --lastMax){
	// do nothing
      }
      if(lastMax != max){
	fprintf(stderr, "in dmaxfilter.cpp, lastMax!=max!\n");
	exit(1);
      }
...
///Max filter imgSrc with current settings and return the result
DImage DMaxFilter::filterImage(const DImage &imgSrc,
...
//  */
// void DMaxFilter::fill_circle_kern_offsets(int radiusX, int radiusY,
...
void DMaxFilter::fill_circle_kern_offsets(int radiusX, int radiusY,
  ...
  if(radiusX != radiusY){
    fprintf(stderr, "DMaxFilter::fill_circle_kern_offsets() radiusX should equal radiusY for now.\n");
    exit(1);
  }
 ...
 * wide and high.
 */
void DMaxFilter::fill_square_kern_offsets(int radiusX, int radiusY,
					     unsigned char *rgKernFull,
					     int *rgRightEdge,
					     int *numKernPxls){
...
void DMaxFilter::maxFilterImage(DImage &imgDst, const DImage &imgSrc,
				      bool fAlreadyPadded,
				      int radiusX, int radiusY,
				      DMaxFiltType filtType, DProgress *pProg,
				      int numThreads){
  DMaxFilter mfilt(radiusX, radiusY, filtType);
